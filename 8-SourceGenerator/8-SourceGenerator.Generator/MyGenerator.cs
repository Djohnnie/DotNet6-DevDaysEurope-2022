                            ...
                            foreach (AdditionalText file in context.AdditionalFiles)
                            {
                                if (file.Path.Contains(swaggerDescriptorPath))
                                {
                                    ...
                                    foreach (var endpoint in swaggerDescription.Endpoints)
                                    {
                                        ...
                                        httpClientBuilder.Append($" RestRequest request = new RestRequest(\"/{endpoint.Name.ToLower()}\");");
                                    ...
                                    httpClientBuilder.Append("} }");
                                    context.AddSource($"{classSymbol.Name}.g.cs", SourceText.From(httpClientBuilder.ToString(), Encoding.UTF8));
                                }
                            }
                        }
